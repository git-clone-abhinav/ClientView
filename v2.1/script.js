Vue.component('client-card', {
  ...
  methods: {
    updateLastUpdate() {
      ...
      this.client.lastUpdate = currentDate;
    },
    openEnlarged() {
  ...
  <img :src="client.file" class="card-img-top">
  <div class="card-body p-1">
    <center>
      <h6 class="card-subtitle m-2 white-text">{{ client.username }}</h6>
      <p class="card-text m-2 white-text" :class="{ 'flash-red': client.flashLastUpdate }">Last Update: {{ client.lastUpdate }}</p>
    </center>
  </div>
</div>
  `
    ...
    connectSocket() {
      ...
      try {
        ...
        this.socket.on('user-screenshot', (data) => {
          ...
          // set lastUpdate to only time
          
          this.clients[data.client].lastUpdate = new Date().toLocaleTimeString();
